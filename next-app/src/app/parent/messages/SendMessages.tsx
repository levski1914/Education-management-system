  ...
  return (
    <div className="flex h-[80vh] p-4">
      ...
      <div className="w-2/3 pl-4 flex flex-col border-l h-full">
        <div
          className="p-2 text-lg font-bold text-white border-b border-blue-500 rounded"
          style={{ backgroundColor: "#1f106f" }}
        >
          {(() => {
            if (!selectedUserId) return "Без съобщения";

            const selectedConversation = conversations.find(
              (c) => c.id === selectedUserId
            );

            if (!selectedConversation) return "Потребител";

            const name = `${selectedConversation.firstName} ${selectedConversation.lastName}`;
            const role = selectedConversation.role || "";
